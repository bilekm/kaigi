...
class ConversationWorkflow(BaseModel):
    ...
    agents: Annotated[list[ConversationAgent], Field(min_length=1)]
    ...
    # - "direct": Single agent executes immediately, no discussion
    collaboration: Literal["team", "orchestrated", "direct"] = "team"
    lead: str | None = None  # Agent ID of lead (required for orchestrated)
    designated_agent: str | None = None  # Agent ID for direct mode (required for direct)
    ...
    def validate_collaboration_requirements(self) -> Self:
        """Validate collaboration mode requirements."""
        agent_ids = [a.id for a in self.agents]

        # Team and orchestrated modes require at least 2 agents
        if self.collaboration in ("team", "orchestrated") and len(self.agents) < 2:
            raise ValueError(
                f"Collaboration mode '{self.collaboration}' requires at least 2 agents. "
                f"Use 'direct' mode for single-agent workflows."
            )
        ...
        if self.collaboration == "direct":
            if not self.designated_agent:
                raise ValueError(
                    "Designated agent must be specified for direct collaboration. "
                    "Add 'designated_agent: <agent-id>' to your workflow."
                )
            if self.designated_agent not in agent_ids:
                raise ValueError(
                    f"Designated agent '{self.designated_agent}' not found. "
                    f"Available agents: {', '.join(agent_ids)}"
                )
