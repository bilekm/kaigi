...
def load_tool_policy(policy_path: Path | str | None = None) -> ToolPolicy:
    ...
    2. .orchestrator/tool_policy.yaml (project-specific)
    3. ~/.orchestrator/tool_policy.yaml (global)
    ...
    project_policy = Path.cwd() / ".orchestrator" / "tool_policy.yaml"
    search_paths.append(project_policy)

    # Global policy
    global_policy = Path.home() / ".orchestrator" / "tool_policy.yaml"
