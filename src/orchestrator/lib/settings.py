...
def get_global_settings_path() -> Path:
    ...
    return Path.home() / ".orchestrator" / "agents.yaml"


def get_project_settings_path() -> Path | None:
    """Get path to project-specific agent settings file.

    Searches current directory and parents for .orchestrator/agents.yaml
    """
    current = Path.cwd()

    while current != current.parent:
        project_settings = current / ".orchestrator" / "agents.yaml"
        if project_settings.exists():
            return project_settings
        current = current.parent

    # Check current directory directly
    direct_path = Path.cwd() / ".orchestrator" / "agents.yaml"
